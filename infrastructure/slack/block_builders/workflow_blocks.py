...
from features.workflows.models import OperationalWorkflow
from infrastructure.slack.block_builders.shared_actions import build_quick_actions

def build_workflow_list_blocks(workflows: List[OperationalWorkflow]) -> List[Dict[str, Any]]:
    blocks = [
        {
            "type": "header",
    ...
    for wf in workflows:
        ...
        stages = wf.definition.get("stages", []) if wf.definition else []
...
def build_workflow_detail_blocks(workflow: OperationalWorkflow) -> List[Dict[str, Any]]:
    ...
    stages = workflow.definition.get("stages", []) if workflow.definition else []
    total_stages = len(stages)
